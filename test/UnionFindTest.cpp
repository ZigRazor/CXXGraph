...
TEST(UnionFindTest, setFindTest1) {
  ...
  */
  ...
  auto subset_ptr = make_shared<std::unordered_map<unsigned long long, CXXGraph::Subset>>(subset);
  // {{0, 0}, {1, 0}, {2, 0}, {3, 0}};
  CXXGraph::Subset set1{0, 0}, set2{1, 0}, set3{2, 0}, set4{3, 0};
  subset = {{0, set1}, {1, set2}, {2, set3}, {3, set4}};

  auto res = graph.setFind(subset_ptr, std::stoi(node0.getUserId()));
  ASSERT_EQ(res, 0);
  res = graph.setFind(subset_ptr, std::stoi(node1.getUserId()));
  ASSERT_EQ(res, 1);
  res = graph.setFind(subset_ptr, std::stoi(node2.getUserId()));
  ASSERT_EQ(res, 2);
}
...
TEST(UnionFindTest, setFindTest2) {
  ...
  auto res = graph.setFind(subset_ptr, std::stoi(node0.getUserId()));
  ASSERT_EQ(res, 0);
  res = graph.setFind(subset_ptr, std::stoi(node3.getUserId()));
  ASSERT_EQ(res, 0);
  res = graph.setFind(subset_ptr, std::stoi(node2.getUserId()));
  ASSERT_EQ(res, 0);
}
...
TEST(UnionFindTest, setUnionTest3) {
  ...
  graph.setUnion(subset_ptr, std::stoi(node1.getUserId()),
  ...
  graph.setUnion(subset_ptr, std::stoi(node2.getUserId()),
                 std::stoi(node3.getUserId()));
  ASSERT_EQ(subset[0].rank, 1);
  ASSERT_EQ(subset[3].parent, 0);
}
...
TEST(UnionFindTest, containsCycle) {
  ...
  CXXGraph::T_EdgeSet<int> edgeSet;
  auto edgeSet_ptr = make_shared<CXXGraph::T_EdgeSet<int>>(edgeSet);
  ...
  CXXGraph::Graph<int> graph(edgeSet);
  bool containsCycle = graph.containsCycle(edgeSet_ptr);
  ASSERT_EQ(containsCycle, false);

  CXXGraph::UndirectedWeightedEdge<int> edge1(1, node1, node2, 10);
  edgeSet.insert(make_shared<CXXGraph::UndirectedWeightedEdge<int>>(edge1));
  containsCycle = graph.containsCycle(edgeSet_ptr);
  ASSERT_EQ(containsCycle, false);
