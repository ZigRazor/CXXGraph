...
const HopcroftKarpResult_struct Graph<T>::hopcroftKarp() const {
  ...
  if (nodeSet.size() < 2) {
    result.errorMessage =
        "Graph must have at least 2 nodes for bipartite matching.";
    return result;
  }
  ...
  std::unordered_map<std::string, int>
      color;  // use node IDs to avoid pointer issues
      ...
      while (!queue.empty() && isBipartite) {
        ...
        for (const auto& neighbor : neighbors) {
          if (color.find(neighbor->getUserId()) == color.end()) {
            ...
            queue.push(neighbor);
          } else if (color[neighbor->getUserId()] ==
                     color[current->getUserId()]) {
  ...
  // Prefer nodes starting with "u" to be in U partition for consistent test
  // results
  int uColor = -1;
  for (const auto& node : nodes) {
