...
namespace CXXGRAPH
{
    ...
    {
        /**
         * @brief A Vertex Cut Partioning Algorithm ( as described by this paper https://arxiv.org/pdf/2010.04414.pdf )
         * @details This algorithm is a greedy algorithm that partitions the graph into n sets of vertices ( as described by this paper https://arxiv.org/pdf/2010.04414.pdf ).
         */
        template <typename T>
        ...
        public:
            explicit WeightBalancedLibra(Globals &G, double _weight_sum_bound, std::unordered_map<std::size_t, int> &&_vertices_degrees);
            ~WeightBalancedLibra();

            void performStep(const Edge<T> &e, PartitionState<T> &Sstate);
        };
        template <typename T>
        WeightBalancedLibra<T>::WeightBalancedLibra(Globals &G, double _weight_sum_bound, std::unordered_map<std::size_t, int> &&_vertices_degrees) 
            : GLOBALS(G), 
        ...
        template <typename T>
        void WeightBalancedLibra<T>::performStep(const Edge<T> &e, PartitionState<T> &state)
        {
            ...
            //Case 2: one or more edges of node v have been assigned but that of node u haven't
            ...
            //Case 3: one or more edges of node u have been assigned but that of node v haven't
            else if (!u_partition.empty() && v_partition.empty())
            {
            ...
            {
                std::shared_ptr<CoordinatedRecord<T>> u_coord_record = std::dynamic_pointer_cast<CoordinatedRecord<T>>(u_record);
                std::shared_ptr<CoordinatedRecord<T>> v_coord_record = std::dynamic_pointer_cast<CoordinatedRecord<T>>(v_record);
                const auto &uv_intersection = u_coord_record->partition_intersection(v_coord_record);
                ...
                if (uv_intersection.empty()) 
                {
                    ...
                    // according to paper, s refers to node with lower degree, t = {u, v} - {s}
                    size_t s_node = (u_degree > v_degree) ? v : u;
                    size_t t_node = (u_degree > v_degree) ? u : v;
                    std::set<int> &s_partition = (u_degree > v_degree) ? v_partition : u_partition;
                    std::set<int> &t_partition = (u_degree > v_degree) ? u_partition : v_partition;
