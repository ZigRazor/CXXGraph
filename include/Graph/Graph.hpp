...
class Graph {
  ...
  */
   ...
   * file
   * @param OFileName The Output File Name ( )
   * @param compress Indicates if the Output will be compressed ( Pay Attention
   * if compress flag is true, not compressed files will be deleted [ #48 ] )
   * @param writeNodeFeat Indicates if export also Node Features
   * @param writeEdgeWeight Indicates if export also Edge Weights
   * @return 0 if all OK, else return a negative value
   */
...
Graph<T>::Graph(const T_EdgeSet<T> &edgeSet) {
  for (const auto &edgeSetIt : edgeSet) {
    ...
    [edgeSetIt](const Edge<T> *edge) { return (*edge == *edgeSetIt); }) ==
    this->edgeSet.end())
    {
...
int Graph<T>::writeToStandardFile(const std::string &workingDir,
  ...
  for (const auto &edge : edgeSet) {
    ofileGraph
        << edge->getId() << separator << edge->getNodePair().first->getUserId()
        << separator << edge->getNodePair().second->getUserId() << separator
        << ((edge->isDirected().has_value() && edge->isDirected().value()) ? 1
                                                                           : 0)
        << std::endl;
  }
  ofileGraph.close();
  ...
  if (writeNodeFeat) {
    ...
    for (const auto &node : nodeSet) {
      ofileNodeFeat << node->getUserId() << separator << node->getData()
                    << std::endl;
    }
    ofileNodeFeat.close();
  }
...
int Graph<T>::writeToDotFile(const std::string &workingDir,
  ...
  for (auto const &edgeIt : edgeSet) {
    ...
    if (edgeIt->isWeighted()) {
      ...
      edgeLine += " [weight" +
                  std::to_string(static_cast<int>(edgeIt->getWeight())) + ']';
    }
    edgeLine += ";\n";
    ofileGraph << edgeLine;
  }
...
int Graph<T>::readFromStandardFile(const std::string &workingDir,
  ...
  std::string nodeId2;
  bool directed;
  while (ifileGraph >> edgeId >> nodeId1 >> nodeId2 >>
  ...
  if (readEdgeWeight) {
    ...
    double weight;
    bool weighted;
    while (ifileEdgeWeight >> edgeId >> weight >>
           weighted) { /* loop continually */
      if (weighted) {
...
int Graph<T>::readFromDotFile(const std::string &workingDir,
  ...
  std::string completePathToFileGraph = workingDir + '/' + fileName ".dot";
  ...
  const std::string directedSeparator = "->";
        if (content.find(directedSeparator) {
    directed = true;
	}
    // Check if the graph is weighted
    bool weighted = false;
	if (content.find("weight")) {
    directed = true;
	}
...
template <typename T>
void Graph<T>::recreateGraphFromReadFiles(
    std::unordered_map<unsigned long long, std::pair<std::string, std::string>>
        &edgeMap,
    std::unordered_map<unsigned long long, bool> &edgeDirectedMap,
    std::unordered_map<std::string, T> &nodeFeatMap,
        ...
        for (const auto &edgeIt : edgeMap) {
          ...
          if (edgeWeightMap.find(edgeIt.first) != edgeWeightMap.end()) {
            if (edgeDirectedMap.find(edgeIt.first) != edgeDirectedMap.end() &&
              ...
              auto edge = new DirectedWeightedEdge<T>(
...
const std::shared_ptr<AdjacencyMatrix<T>> Graph<T>::getAdjMatrix() const {
        ...
        for (const auto &edgeSetIt : edgeSet) {
          if (edgeSetIt->isDirected().has_value() &&
            ...
            addElementToAdjMatrix(&(d_edge->getFrom()), &(d_edge->getTo()),
                                  d_edge);
            ...
            ;
            addElementToAdjMatrix(&(ud_edge->getNode1()),
                                  &(ud_edge->getNode2()), ud_edge);
            addElementToAdjMatrix(&(ud_edge->getNode2()),
                                  &(ud_edge->getNode1()), ud_edge);
...
const DijkstraResult Graph<T>::dijkstra(const Node<T> &source,
        ...
        if (std::find(nodeSet.begin(), nodeSet.end(), &source) ==
            nodeSet.end()) {
          // check if source node exist in the graph
          result.errorMessage = ERR_SOURCE_NODE_NOT_IN_GRAPH;
          return result;
        }
        if (std::find(nodeSet.begin(), nodeSet.end(), &target) ==
            nodeSet.end()) {
              ...
              if (elem.second->isWeighted().has_value() &&
                ...
                if (elem.second->isDirected().has_value() &&
                  ...
                  const DirectedWeightedEdge<T> *dw_edge =
                      dynamic_cast<const DirectedWeightedEdge<T> *>(
                          elem.second);
                  if (dw_edge->getWeight() < 0) {
                    result.errorMessage = ERR_NEGATIVE_WEIGHTED_EDGE;
                    return result;
                  } else if (currentDist + dw_edge->getWeight() <
                             dist[elem.first]) {
                  ...
                  const UndirectedWeightedEdge<T> *udw_edge =
                      dynamic_cast<const UndirectedWeightedEdge<T> *>(
                          elem.second);
                  if (udw_edge->getWeight() < 0) {
                    result.errorMessage = ERR_NEGATIVE_WEIGHTED_EDGE;
                    return result;
                  } else if (currentDist + udw_edge->getWeight() <
                             dist[elem.first]) {
...
const FWResult Graph<T>::floydWarshall() const {
        ...
        for (const auto &edge : edgeSet) {
          ...
          if (edge->isWeighted().has_value() && edge->isWeighted().value()) {
            auto edgeWeight =
                (dynamic_cast<const Weighted *>(edge))->getWeight();
            auto key = std::make_pair(elem.first->getUserId(),
                                      elem.second->getUserId());
...
const MstResult Graph<T>::prim() const {
        ...
        while (!pq.empty()) {
          ...
          if (std::find(doneNode.begin(), doneNode.end(), nodeId) ==
              doneNode.end()) {
            auto pair =
                std::make_pair(parentNode[nodeId], currentNode->getUserId());
...
const MstResult Graph<T>::boruvka() const {
        ...
        while (numTrees > 1) {
          ...
          for (const auto &[nodeId, edgeId] : cheapest) {
            ...
            if (edgeId != INT_MAX) {
              auto cheapestNode =
                  Graph<T>::getEdge(edgeId).value()->getNodePair();
              auto set1 =
                  Graph<T>::setFind(&subsets, cheapestNode.first->getId());
              auto set2 =
                  Graph<T>::setFind(&subsets, cheapestNode.second->getId());
              if (set1 == set2) continue;
              result.mstCost += edgeWeight[edgeId];
              auto newEdgeMST =
                  std::make_pair(cheapestNode.first->getUserId(),
...
const std::vector<Node<T>> Graph<T>::breadth_first_search(
    const Node<T> &start) const {
        ...
        if (std::find(nodeSet.begin(), nodeSet.end(), &start) ==
            nodeSet.end()) {
          return visited;
        }
        const auto adj = Graph<T>::getAdjMatrix();
          ...
          while (!level_tracker.empty()) {
            ...
            if (num_threads == 1 + waiting_workers.fetch_add(1)) {
              ...
              if (level_tracker.size() <= num_threads * 4) {
                block_size = std::max(
                    1, static_cast<int>(
                           std::ceil(static_cast<double>(level_tracker.size()) /
                                     num_threads)));
...
const std::vector<Node<T>> Graph<T>::depth_first_search(
    const Node<T> &start) const {
        ...
        std::function<void(const std::shared_ptr<AdjacencyMatrix<T>>,
                           const Node<T> &, std::vector<Node<T>> &)>
            explore;
...
bool Graph<T>::isCyclicDirectedGraphDFS() const {
        ...
        for (const auto &node : nodeSet) {
          ...
          if (state[node->getId()] == not_visited) {
            // Check for cycle.
            std::function<bool(
                const std::shared_ptr<AdjacencyMatrix<T>>,
                ...
                [this, &isCyclicDFSHelper](
                  ...
                  if (it != adjMatrix->end()) {
                    for (const auto &child : it->second) {
                      ...
                      auto state_of_child =
                          states.at((std::get<0>(child))->getId());
          ...
          for (const auto &start_node : nodeSet) {
            ...
            std::function<void(const Node<T> *)> dfs_helper =
                [this, &adjMatrix, &visited,
                 &dfs_helper](const Node<T> *source) {
                  // mark the vertex visited
                  visited[source->getId()] = true;
...
SCCResult<T> Graph<T>::kosaraju() const {
        ...
        if (!isDirectedGraph()) {
          ...
          for (const auto &edgeSetIt : edgeSet) {
            ...
            addElementToAdjMatrix(&(d_edge->getTo()), &(d_edge->getFrom()),
                                  d_edge);
          }

          visited.clear();

          std::function<void(const Node<T> *, std::vector<Node<T>> &)>
              dfs_helper1 =
...
const DialResult Graph<T>::dial(const Node<T> &source, int maxWeight) const {
        ...
        std::unordered_map<const Node<T> *, std::pair<long, const Node<T> *>>
            dist;
        ...
        while (1) {
          ...
          for (const auto &i : (*adj)[u]) {
            ...
            auto u_i = std::find_if(
                dist.begin(), dist.end(),
                [u](std::pair<const Node<T> *,
                              std::pair<long, const Node<T> *>> const &it) {
                  return (*u == *(it.first));
                });

            auto v_i = std::find_if(
                dist.begin(), dist.end(),
                [v](std::pair<const Node<T> *,
                              std::pair<long, const Node<T> *>> const &it) {
                  return (*v == *(it.first));
                });
            ...
            if (dv > du + weight) {
              ...
              if (dv != std::numeric_limits<long>::max()) {
                auto findIter =
                    std::find(B[dv].begin(), B[dv].end(), dist[v].second);
                B[dv].erase(findIter);
              }
...
double Graph<T>::fordFulkersonMaxFlow(const Node<T> &source,
        ...
        auto bfs_helper = [this, &source, &target, &parent,
                           &weightMap]() -> bool {
          std::unordered_map<const Node<T> *, bool> visited;
...
const std::vector<Node<T>> Graph<T>::graph_slicing(const Node<T> &start) const {
        ...
        for (auto const &node : C1) {
          std::vector<Node<T>> reachableNodes =
              Graph<T>::depth_first_search(*node);
...
int Graph<T>::writeToFile(InputOutputFormat format,
        ...
        int result = 0;
        result = writeToStandardFile(workingDir, OFileName, compress,
                                     writeNodeFeat, writeEdgeWeight, format);
...
int Graph<T>::readFromFile(InputOutputFormat format,
        ...
        if (result == 0) {
          result = readFromStandardFile(workingDir, OFileName, compress,
                                        readNodeFeat, readEdgeWeight, format);
        }
        return result;
}
...
PartitionMap<T> Graph<T>::partitionGraph(
    ...
    const unsigned int numberOfThreads) const {
        ...
        PARTITIONING::Globals globals(numberOfPartitions, algorithm, param1,
                                      param2, param3, numberOfThreads);
